...
class TrayGenerator {
  ...
  toggleWindow = () => {
    // Clicking the tray icon blurs the window (hiding it) just before this
    // click handler runs, so guard against immediately reshowing it.
    const justHidByBlur = Date.now() - (this.mainWindow.lastHideTime || 0) < 250;

    if (this.mainWindow.isVisible()) {
      this.mainWindow.hide();
    } else if (!justHidByBlur) {
      this.setWinPosition();
      this.showWindow();
